...
def home(request):
	...
	verified = investments.filter(status='Verified').count()
...
def watchus(request):
	...
	return render(request, 'accounts/watchus.html')	
...
def makeInvestment(request, pk):
	...
	if request.method == 'POST':
		...
		formset = InvestmentFormSet(request.POST, instance=investor)
		if formset.is_valid():
			formset.save()
			return redirect('/')
...
def userInvestment(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return redirect('https://paystack.com/pay/9590qju3l0')
